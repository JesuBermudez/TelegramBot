export default async function laughter(ctx, bot) {
    ...
    // final result
    let result = ""
    // id of the message that is replying to
    let replyId = false
    // extra message
    let extra = ""
    // what follows after the command
    const rightText = ctx.payload.trim().split(" ")
    ...
    // send the message
    if (replyId) {
        ctx.reply(`De: @${user}, ${extra} ${result}`, {reply_to_message_id: replyId})
    } else {
